...
1. Since we modified `composer.json` we will need to update Composer. This will also download the defined dependencies:
    ...
    <Image alt="image of terminal running a composer install" path="guides/drupal-8-composer-no-ci/drops-8-composer-update.png" />
...
5. Now let’s run `git status` again to make sure everything is included:
   ...
   <Image alt="Image of git status showing the changed files in red" path="guides/drupal-8-composer-no-ci/drops-8-composer-git-status-after-installing-d8.png" />
...
4. Log in to your new environment and verify that the address module exists:
   ...
   <Image alt="Image of installing address module" path="guides/drupal-8-composer-no-ci/drops-8-composer-drupal-8-address-module-install.png" />
